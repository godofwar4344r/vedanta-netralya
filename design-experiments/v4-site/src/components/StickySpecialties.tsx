...
const SPECS: Spec[] = [
  { num: '01', tag: 'Robotic FLACS Laser', title: 'Cataract Surgery', desc: 'Premium micro-incision phacoemulsification with foldable multifocal and toric IOLs. Painless, with visual recovery inside 24 hours.', stat: '15,000+', statLabel: 'Lens implants', stat2: '24 hrs', stat2Label: 'Recovery', img: serviceCataract, path: '/cataract' },
  ...
  { num: '03', tag: 'Squint and Orthoptics', title: 'Pediatric Ophthalmology', desc: 'Gentle, specialised eye care for infants and children, squint correction and structured vision-therapy programs.', stat: '5,000+', statLabel: 'Children treated', stat2: '0-16', stat2Label: 'Age range', img: servicePaediatric, path: '/paediatric-ophthalmology' },
  ...
  { num: '05', tag: 'Vitreoretinal Specialty', title: 'Retina and Vitreous', desc: 'Sutureless surgery for diabetic retinopathy, macular degeneration, and retinal detachment.', stat: '6,000+', statLabel: 'Procedures', stat2: '25G', stat2Label: 'Sutureless', img: serviceRetina, path: '/retina-services' },
];
      ...
      <div className="lg:sticky lg:top-0 lg:h-screen flex items-center py-20 lg:py-0">
        <div className="max-w-[1800px] mx-auto px-6 lg:px-16 w-full">
          <div className="mb-10 lg:mb-12">
            ...
            <h2 className="section-text text-brand-navy leading-[1.02] tracking-tight">
              Five wings of <span className="italic font-light text-brand-teal">ophthalmic excellence.</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
            <div className="hidden lg:flex flex-col">
              {SPECS.map((s, i) => (
                <button key={i} onClick={() => jumpTo(i)} className="group text-left py-4 border-b border-brand-navy/10 flex items-center gap-5 transition-all duration-300" style={{ paddingLeft: active === i ? 18 : 0 }}>
                  <span className="block rounded-full transition-all duration-300" style={{ width: 4, height: active === i ? 40 : 18, background: active === i ? 'var(--brand-teal)' : 'rgba(10,38,64,0.15)' }} />
                  ...
                  <span className="font-black tracking-tight transition-all duration-300" style={{ fontSize: active === i ? '2.25rem' : '1.6rem', lineHeight: 1.05, color: active === i ? 'var(--brand-navy)' : 'rgba(10,38,64,0.34)' }}>{s.title}</span>
                </button>
              ))}
            ...
            <div className="relative">
              <div key={active} className="fade-up rounded-[2rem] overflow-hidden bg-white border border-brand-navy/10 shadow-[0_40px_80px_-50px_rgba(10,38,64,0.6)]">
            ...
            <div className="flex flex-col gap-3 lg:hidden">
              {SPECS.map((s, i) => (
                <button key={i} onClick={() => setActive(i)} className={`text-left rounded-2xl border p-4 transition-all ${active === i ? 'bg-white border-brand-teal/40 shadow-lg' : 'bg-white/50 border-brand-navy/10'}`}>
                  ...
                  <h4 className="text-lg font-black text-brand-navy" style={{ fontFamily: "'General Sans', sans-serif" }}>{s.title}</h4>
                </button>
              ))}
            </div>
          </div>
